...
def evaluate_pending_entry_watch(service, recent_1m_candles):
    ...
    return _evaluate_triggered_watch(service, pending, latest, previous, recent_1m_candles, direction, volume_ok, hybrid_mode, fast_track_ready, elite_watch_ready)


def _evaluate_triggered_watch(service, pending, latest, previous, recent_1m_candles, direction, volume_ok, hybrid_mode, fast_track_ready, elite_watch_ready):
    ...
    if not momentum_quality["ok"] and not (fast_track_ready or hybrid_mode or elite_watch_ready):
        if service._pending_watch_retrigger_eligible(pending, latest, previous):
