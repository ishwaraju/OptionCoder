...
class ConnectionManager:
    ...
    def get_connection_state(self):
        """Get current connection state for data collector"""
        live_snapshot = self.live_feed.get_live_data()
        return {
            "recovered": False,
            "missing_candles": [],
            "feed_connected": live_snapshot.get("feed_connected", False),
            "data_age_seconds": live_snapshot.get("data_age_seconds", 0),
            "reconnect_attempts": live_snapshot.get("connection_diagnostics", {}).get("reconnect_attempts", 0),
            "stability_score": live_snapshot.get("connection_diagnostics", {}).get("stability_score", 0),
        }
