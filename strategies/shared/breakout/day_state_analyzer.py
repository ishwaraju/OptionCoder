...
class DayStateAnalyzer:
    ...
    def derive_active_day_state(recent_candles_5m, price, vwap, atr, pressure_metrics, opening_bias, time_regime):
        ...
        close_rises, close_falls = DayStateAnalyzer.slope_counts(closes[-5:] if len(closes) >= 5 else closes)
        high_rises, high_falls = DayStateAnalyzer.slope_counts(highs[-5:] if len(highs) >= 5 else highs)
        low_rises, low_falls = DayStateAnalyzer.slope_counts(lows[-5:] if len(lows) >= 5 else lows)
...
def slope_counts(values):
    return DayStateAnalyzer.slope_counts(values)


def derive_opening_bias(recent_candles_5m, vwap, atr, trade_start, first_30_end, first_60_end):
    return DayStateAnalyzer.derive_opening_bias(recent_candles_5m, vwap, atr, trade_start, first_30_end, first_60_end)


def derive_active_day_state(recent_candles_5m, price, vwap, atr, pressure_metrics, opening_bias, time_regime):
    return DayStateAnalyzer.derive_active_day_state(recent_candles_5m, price, vwap, atr, pressure_metrics, opening_bias, time_regime)


def apply_day_state_adjustment(score, scored_direction, cautions, components, day_state, append_cautions):
    return DayStateAnalyzer.apply_day_state_adjustment(score, scored_direction, cautions, components, day_state, append_cautions)
