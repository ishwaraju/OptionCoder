...
class ContinuationEngine:
    ...
    def evaluate_core_continuations(strategy, ctx):
        ...
        relaxed_high_continuation_score = max(time_thresholds["high_continuation_min_score"] - 4, 72)
        relaxed_continuation_score = max(time_thresholds["continuation_min_score"] - 4, 68)

        if scored_direction == "CE" and high_score_continuation_ready(price_vwap_aligned=price > vwap, volume_ok=strategy._sensex_volume_flexible(volume_signal, score, time_regime, candle_range, atr), score=score, high_continuation_min_score=relaxed_high_continuation_score if acceptance_context_ready else time_thresholds["high_continuation_min_score"], oi_bias_ok=oi_bias in ["BULLISH", "NEUTRAL"], oi_trend_ok=oi_trend in ["BULLISH", "NEUTRAL", None], build_up_ok=bullish_build_up_ok, orb_extension_ok=(orb_high is None or price <= orb_high + (buffer * tuning["extension_buffer_mult"])), candle_liquidity_ok=candle_liquidity_ok, breakout_body_ok=breakout_body_ok, breakout_structure_ok=breakout_structure_ok, ha_ok=bullish_ha_ok, far_from_vwap_ok=("far_from_vwap" not in cautions or (score >= 82 and volume_signal == "STRONG" and pressure_metrics and pressure_metrics["pressure_bias"] == "BULLISH") or continuation_override_ok or acceptance_context_ready), opposite_pressure_ok=("opposite_pressure" not in cautions or continuation_override_ok), continuation_regime_ok=continuation_regime_ok, adx_trade_ok=(adx_trade_ok or acceptance_context_ready), mtf_trade_ok=(mtf_trade_ok or acceptance_context_ready), pressure_conflict_ok=(pressure_conflict_level in {"NONE", "MILD"} or recent_breakout_context or acceptance_context_ready), focused_mode_ok=(not Config.FOCUSED_MANUAL_MODE or recent_breakout_context or (time_regime in ["OPENING", "MID_MORNING"] and score >= 78 and volume_signal == "STRONG") or acceptance_context_ready)):
            message = "Acceptance-backed bullish impulse continuation" if acceptance_context_ready else "High-score bullish continuation"
            return strategy._emit_trade_signal("CE", "CONTINUATION", score, volume_signal, pressure_metrics, cautions, blockers=blockers, regime=strategy._effective_signal_regime(expiry_eval, regime), candle_time=candle_time, message=message, trigger_price=price, invalidate_price=vwap, atr=atr, support=support, resistance=resistance, reset_retest=True, reset_confirmation=True)

        if scored_direction == "CE" and fallback_continuation_ready(price_vwap_aligned=price > vwap, volume_ok=strategy._sensex_volume_flexible(volume_signal, score, time_regime, candle_range, atr), score=score, continuation_min_score=relaxed_continuation_score if acceptance_context_ready else time_thresholds["continuation_min_score"], oi_bias_ok=oi_bias in ["BULLISH", "NEUTRAL"], oi_trend_ok=oi_trend in ["BULLISH", "NEUTRAL", None], build_up_ok=bullish_build_up_ok, candle_liquidity_ok=candle_liquidity_ok, breakout_body_ok=breakout_body_ok, breakout_structure_ok=breakout_structure_ok, ha_ok=bullish_ha_ok, opposite_pressure_ok=("opposite_pressure" not in cautions or continuation_override_ok), far_from_vwap_ok=("far_from_vwap" not in cautions or continuation_override_ok or acceptance_context_ready), opening_session=opening_session, continuation_regime_ok=continuation_regime_ok, adx_ok=(adx_trade_ok or recent_breakout_context or continuation_override_ok or acceptance_context_ready), mtf_ok=(mtf_trade_ok or recent_breakout_context or continuation_override_ok or acceptance_context_ready), pressure_conflict_ok=(recent_breakout_context or pressure_conflict_level == "NONE" or continuation_override_ok or acceptance_context_ready), focused_mode_ok=(not Config.FOCUSED_MANUAL_MODE or recent_breakout_context or continuation_override_ok or acceptance_context_ready)):
            message = "Acceptance-backed continuation follow-through" if acceptance_context_ready else "Continuation follow-through setup"
            return strategy._emit_trade_signal("CE", "CONTINUATION", score, volume_signal, pressure_metrics, cautions, blockers=blockers, regime=strategy._effective_signal_regime(expiry_eval, regime), candle_time=candle_time, message=message, trigger_price=price, invalidate_price=vwap, atr=atr, support=support, resistance=resistance, reset_retest=True, reset_confirmation=True)

        if scored_direction == "PE" and high_score_continuation_ready(price_vwap_aligned=price < vwap, volume_ok=strategy._sensex_volume_flexible(volume_signal, score, time_regime, candle_range, atr), score=score, high_continuation_min_score=relaxed_high_continuation_score if acceptance_context_ready else time_thresholds["high_continuation_min_score"], oi_bias_ok=oi_bias in ["BEARISH", "NEUTRAL"], oi_trend_ok=oi_trend in ["BEARISH", "NEUTRAL", None], build_up_ok=bearish_build_up_ok, orb_extension_ok=(orb_low is None or price >= orb_low - (buffer * tuning["extension_buffer_mult"])), candle_liquidity_ok=candle_liquidity_ok, breakout_body_ok=breakout_body_ok, breakout_structure_ok=breakout_structure_ok, ha_ok=bearish_ha_ok, far_from_vwap_ok=("far_from_vwap" not in cautions or (score >= 82 and volume_signal == "STRONG" and pressure_metrics and pressure_metrics["pressure_bias"] == "BEARISH") or continuation_override_ok or acceptance_context_ready), opposite_pressure_ok=("opposite_pressure" not in cautions or continuation_override_ok), continuation_regime_ok=continuation_regime_ok, adx_trade_ok=(adx_trade_ok or acceptance_context_ready), mtf_trade_ok=(mtf_trade_ok or acceptance_context_ready), pressure_conflict_ok=(pressure_conflict_level in {"NONE", "MILD"} or recent_breakout_context or acceptance_context_ready), focused_mode_ok=(not Config.FOCUSED_MANUAL_MODE or recent_breakout_context or (time_regime in ["OPENING", "MID_MORNING"] and score >= 78 and volume_signal == "STRONG") or acceptance_context_ready)):
            message = "Acceptance-backed bearish impulse continuation" if acceptance_context_ready else "High-score bearish continuation"
            return strategy._emit_trade_signal("PE", "CONTINUATION", score, volume_signal, pressure_metrics, cautions, blockers=blockers, regime=strategy._effective_signal_regime(expiry_eval, regime), candle_time=candle_time, message=message, trigger_price=price, invalidate_price=vwap, atr=atr, support=support, resistance=resistance, reset_retest=True, reset_confirmation=True)

        if scored_direction == "PE" and fallback_continuation_ready(price_vwap_aligned=price < vwap, volume_ok=strategy._sensex_volume_flexible(volume_signal, score, time_regime, candle_range, atr), score=score, continuation_min_score=relaxed_continuation_score if acceptance_context_ready else time_thresholds["continuation_min_score"], oi_bias_ok=oi_bias in ["BEARISH", "NEUTRAL"], oi_trend_ok=oi_trend in ["BEARISH", "NEUTRAL", None], build_up_ok=bearish_build_up_ok, candle_liquidity_ok=candle_liquidity_ok, breakout_body_ok=breakout_body_ok, breakout_structure_ok=breakout_structure_ok, ha_ok=bearish_ha_ok, opposite_pressure_ok=("opposite_pressure" not in cautions or continuation_override_ok), far_from_vwap_ok=("far_from_vwap" not in cautions or continuation_override_ok or acceptance_context_ready), opening_session=opening_session, continuation_regime_ok=continuation_regime_ok, adx_ok=(adx_trade_ok or recent_breakout_context or continuation_override_ok or acceptance_context_ready), mtf_ok=(mtf_trade_ok or recent_breakout_context or continuation_override_ok or acceptance_context_ready), pressure_conflict_ok=(recent_breakout_context or pressure_conflict_level == "NONE" or continuation_override_ok or acceptance_context_ready), focused_mode_ok=(not Config.FOCUSED_MANUAL_MODE or recent_breakout_context or continuation_override_ok or acceptance_context_ready)):
            message = "Acceptance-backed continuation follow-through" if acceptance_context_ready else "Continuation follow-through setup"
            return strategy._emit_trade_signal("PE", "CONTINUATION", score, volume_signal, pressure_metrics, cautions, blockers=blockers, regime=strategy._effective_signal_regime(expiry_eval, regime), candle_time=candle_time, message=message, trigger_price=price, invalidate_price=vwap, atr=atr, support=support, resistance=resistance, reset_retest=True, reset_confirmation=True)
...
def evaluate_core_continuations(strategy, ctx):
    return ContinuationEngine.evaluate_core_continuations(strategy, ctx)


def evaluate_aggressive_continuations(strategy, ctx):
    return ContinuationEngine.evaluate_aggressive_continuations(strategy, ctx)
